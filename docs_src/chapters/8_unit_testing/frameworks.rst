...
Pytest is a great testing framework, and can be installed via ``pip``. A
...
A nice feature is that the command ``pytest`` will run all the tests that it can discover, even
if they were written with ``unittest``. This compatibility makes it easier to transition gradually.
...
it with the help of ``pytest`` fixtures.
...
In the case of ``pytest``, we have to install the ``pytest-cov`` package. Once installed, when the tests are
run, we have to tell the ``pytest`` runner that ``pytest-cov`` will also run, and which package (or packages)
